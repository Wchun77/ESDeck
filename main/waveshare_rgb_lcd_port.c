...
void waveshare_esp32_s3_touch_reset()
{
    ...
    /* CH422G 設定為輸出模式，寫入 0x01 到位址 0x24 */
    uint8_t usb_sel_buf = 0x01;
    esp_err_t ret = i2c_master_write_to_device(I2C_MASTER_NUM, 0x24, &usb_sel_buf, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
    ESP_LOGI("CH422G", "Set system param: %s", esp_err_to_name(ret));

    /* 設定 CH422G 輸出暫存器（位址 0x38），值 0x0E 使 EXIO5=0
    * EXIO5 控制 U9 FSUSB42UMX 的 SEL 腳
    * SEL=0 → HSD1 → ESP_USBH_N/P（GPIO19/20，ESP OTG）接通至 Type_C2
    * SEL=1 → HSD2 → CAN bus
    * 此設定讓 USB 孔走 ESP OTG，TinyUSB HID 才能被 PC 偵測到 */
    usb_sel_buf = 0x1A;
    ret = i2c_master_write_to_device(I2C_MASTER_NUM, 0x38, &usb_sel_buf, 1, I2C_MASTER_TIMEOUT_MS / portTICK_PERIOD_MS);
    ESP_LOGI("CH422G", "Set IO output 0x1E: %s", esp_err_to_name(ret));
}
